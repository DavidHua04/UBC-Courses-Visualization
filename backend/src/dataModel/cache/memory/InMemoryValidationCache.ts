 ...
 * so it can be swapped with a different cache backend later (Redis, etc.)
 * without touching any service code — add a sibling folder under
 * `dataModel/cache/<name>/` and a switch case in `dataModel/index.ts`.
 *
 * Why we need a cache:
 ...
 *
 * Why "in-memory"?
 *   Keeps the dependency footprint small — no external service required.
 *   The trade-off: the cache is lost on every server restart, but that's
 ...
 *
 * All methods are `async` to match the interface (a remote cache
 * implementation would genuinely be async), even though Map operations
 * are synchronous.
 */
