...
export class JsonPlanEntryRepository implements IPlanEntryRepository {
   ...
   *      throw an error with code "23505" (the conventional unique-violation
   *      error code, so a future SQL backend's native error can be handled
   *      uniformly by the route layer).
  ...
  async create(entry: NewEntry): Promise<EntryRow> {
    ...
    if (duplicate) {
      // Error code "23505" is the conventional unique_violation code so the
      // route handler can treat all storage backends uniformly
